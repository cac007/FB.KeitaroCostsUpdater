...
class csvCRUD{
     ...
     * @return string
     */
    function get_input_by_column($col,$input_name,$default_val='',$read_only=false){
		if(array_key_exists($col,$this->form_input_templates)){
			...
			$i = "<input type=\"text\" name=\"$input_name\" value=\"$default_val\" ". $ro." />";
		}
		return $i;
    }
    ...
    function get_add_record_form(){
        ...
        $html .= "<table width=\"100%\">\n<tr><th colspan=\"2\">$header</th></tr>\n<tr><th>Колонка:</th><th>Значение:</th></tr>\n";
            foreach($this->rows[0] as $key => $val){
                ...
                $cell = $key.'|'.$row;
                $th = (array_key_exists($key,$this->custom_tbl_headers)) ? $this->custom_tbl_headers[$key] : 'Колонка: '.$key ;
                $input = $this->get_input_by_column($key,"add_cells[$cell]");
                $html .= "<tr><td class=\"frm_lbl\">".$th."</td><td class=\"frm_input\">$input</td></tr>\n";
            }
        $html .= "<tr><td colspan=\"2\" align=\"right\"><input type=\"hidden\" name=\"row\" value=\"$row\" /><input type=\"hidden\" name=\"action\" value=\"add\" /><input type=\"button\" name=\"cancel\" value=\"Отмена\" onclick=\"location.href='".$this->default_url."'\" />&nbsp;&nbsp;<input type=\"submit\" name=\"submit\" value=\"Добавить\" /></td></tr>\n";
        $html .= "</table>\n</form>\n";
        return $html;
    }
    ...
    function get_row_edit_form($row){
		$header = "Редактирование записи #".$row;
        $html = "<form name=\"frm_csvEdit\" action=\"\" method=\"POST\" enctype=\"multipart/form-data\">\n";
        ...
        $html .= "<tr><td colspan=\"2\" align=\"right\"><br><br><input type=\"hidden\" name=\"row\" value=\"$row\" /><input type=\"hidden\" name=\"action\" value=\"edit\" /><input type=\"button\" name=\"cancel\" value=\"Отмена\" onclick=\"location.href='".$this->default_url."'\" />&nbsp;&nbsp;<input type=\"submit\" name=\"submit\" value=\"Подтвердить\" /></td></tr>\n";
        $html .= "</table>\n</form>\n";
        return $html;
    }
	
	/**
     * csvCRUD::get_row_delete_form()
     * @desc Create a form to delete record
     * @param int $row
     * @return string
     */
    function get_row_delete_form($row){
		$header = "Удаление записи #".$row;
        $html = "<form name=\"frm_csvDelete\" action=\"\" method=\"POST\" enctype=\"multipart/form-data\">\n";
        $html .= "<table width=\"100%\">\n<tr><th colspan=\"2\">$header<hr><br></th></tr>\n";
            foreach($this->rows[$row] as $key => $val){
                if($key == 'RowNum') continue;
                //if(in_array($key,$this->hide_cols)) continue;
                $cell = $key.'|'.$row;
                $th = (array_key_exists($key,$this->custom_tbl_headers)) ? $this->custom_tbl_headers[$key] : 'Column: '.$key ;
                $input = $this->get_input_by_column($key,"update_cells[$cell]",$val,true);
                $html .= "<tr><td class=\"frm_lbl\">".$th."</td><td>$input</td></tr>\n";
            }
        $html .= "<tr><td colspan=\"2\" align=\"right\"><br><br><input type=\"hidden\" name=\"row\" value=\"$row\" /><input type=\"hidden\" name=\"action\" value=\"delete\" /><input type=\"button\" name=\"cancel\" value=\"Отмена\" onclick=\"location.href='".$this->default_url."'\" />&nbsp;&nbsp;<input type=\"submit\" name=\"submit\" value=\"Подтвердить\" /></td></tr>\n";
        $html .= "</table>\n</form>\n";
		return $html;
    }
